...
/** Handles MySQL-specific schema transformations to ensure compatibility with ScyllaDB/CQL types
  ...
  *
  * MySQL type -> Spark JDBC type -> CQL type mapping: INT/BIGINT -> IntegerType/LongType ->
  * int/bigint VARCHAR/TEXT -> StringType -> text DECIMAL -> DecimalType -> decimal
  * DATETIME/TIMESTAMP -> TimestampType -> timestamp BLOB/BINARY -> BinaryType -> blob
  * BOOLEAN/TINYINT(1) -> BooleanType/IntegerType -> boolean/int FLOAT/DOUBLE ->
  * FloatType/DoubleType -> float/double DATE -> DateType -> date JSON -> StringType -> text ENUM ->
  * StringType -> text BIT -> BinaryType/BooleanType -> blob/boolean UNSIGNED BIGINT -> DecimalType
  * -> varint YEAR -> IntegerType/DateType -> int TIME -> TimestampType -> text (as duration string)
  * SET -> StringType -> text
  */
object MySQLSchemaMapper {
  ...
  /** Apply MySQL-specific type transformations to a DataFrame to ensure compatibility with the
    * ScyllaDB writer.
    *
    * Most MySQL types map cleanly through Spark's JDBC layer, but some need explicit handling:
  ...
  /** Check whether a binary column consistently contains single-byte values, which indicates it's
    * likely a MySQL BIT(1) column. Uses a sampling approach to avoid scanning the entire table.
    */
